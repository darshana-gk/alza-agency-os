...
function normalizeStatus(value: string | null): MGAStatus {
...
export function MGAs() {
  ...
  const [selected, setSelected] = useState<MGA | null>(null)
  ...
  const loadRows = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from('mgas')
      .select('id, mga_name, contact_person, email, phone, status, states, lines_of_business, notes, archived_at')
      .is('archived_at', null)
    ...
    if (error) {
      ...
      setRows(
        (data ?? []).map((row) => ({
          id: row.id as string,
          name: String(row.mga_name ?? '').trim() || '—',
          contactPerson: String(row.contact_person ?? '').trim(),
          email: String(row.email ?? '').trim(),
          phone: String(row.phone ?? '').trim(),
          states: String(row.states ?? '').trim(),
          linesOfBusiness: String(row.lines_of_business ?? '').trim(),
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      return (
        ...
        r.states.toLowerCase().includes(q) ||
  ...
  function openEdit(mga: MGA) {
    setSelected(mga)
    setForm({
      mgaName: mga.name,
      contactPerson: mga.contactPerson,
      email: mga.email,
      phone: mga.phone,
      status: mga.status,
      states: mga.states,
      linesOfBusiness: mga.linesOfBusiness,
      notes: mga.notes,
    })
    setFormError(null)
    setArchiveConfirm(false)
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (modalMode === 'add') {
      const result = await createMga(form)
      setSaving(false)
      if (result.error) {
  ...
  async function handleArchive() {
    ...
    setSaving(true)
    setFormError(null)
    const result = await archiveMga(selected.id)
    setSaving(false)
    if (result.error) {
      setFormError(`RLS/query error on ${result.error.table} (${result.error.operation}): ${result.error.message}`)
      setArchiveConfirm(false)
    ...
    <div className="space-y-6">
      ...
      {fetchError && (
        <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
          Failed to load MGAs: {fetchError}
        </div>
      )}
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                paginated.map((mga) => (
                  <tr
                    key={mga.id}
                    onClick={() => canMutate && openEdit(mga)}
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-sm text-slate-700">{mga.contactPerson || '—'}</td>
                    <td className="px-6 py-4 text-sm text-slate-700">{mga.email || '—'}</td>
                    <td className="px-6 py-4 text-sm text-slate-700">{mga.states || '—'}</td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ring-1 ring-inset ${statusStyles[mga.status]}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg rounded-xl border border-slate-200 bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
              <h3 className="text-lg font-semibold text-slate-900">{modalMode === 'add' ? 'Add MGA' : 'Edit MGA'}</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 px-5 py-4">
              ...
              <div>
                <label className="mb-1 block text-xs font-medium uppercase tracking-wider text-slate-500">MGA name *</label>
                <input
                  required
                  disabled={modalMode === 'edit'}
                  className={`${inputClassName} ${modalMode === 'edit' ? 'bg-slate-50 text-slate-600' : ''}`}
                  value={form.mgaName}
                  onChange={(e) => setForm((f) => ({ ...f, mgaName: e.target.value }))}
                />
                {modalMode === 'edit' && (
                  <p className="mt-1 text-xs text-slate-500">
                    Name is locked because policies and transactions store the MGA as text.
                  </p>
                )}
              </div>
              <div>
                <label className="mb-1 block text-xs font-medium uppercase tracking-wider text-slate-500">Contact person</label>
                <input className={inputClassName} value={form.contactPerson} onChange={(e) => setForm((f) => ({ ...f, contactPerson: e.target.value }))} />
              </div>
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <label className="mb-1 block text-xs font-medium uppercase tracking-wider text-slate-500">States</label>
                  <input className={inputClassName} value={form.states} onChange={(e) => setForm((f) => ({ ...f, states: e.target.value }))} />
                </div>
                <div>
              ...
              {modalMode === 'edit' && archiveConfirm && (
                <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-3 text-sm text-amber-900">
                  <p className="font-medium">Archive this MGA?</p>
