...
const MATRIX: Record<string, TransactionDateSemantics> = {
  ...
  endorsement_premium: {
    ...
    txnEffectiveLabel: 'Transaction Effective Date',
    txnExpirationLabel: 'Transaction Expiration Date',
    notes: 'Applies to the endorsement only. The existing policy term is not changed.',
  },
  audit_premium: {
    type: 'audit_premium',
    label: 'Audit',
  ...
  cancellation_premium: {
    ...
    txnExpirationLabel: 'Transaction Expiration Date',
    notes:
      'Cancellation effective date is stored on the transaction. The original policy effective date is not overwritten.',
  },
  return_premium: {
    type: 'return_premium',
