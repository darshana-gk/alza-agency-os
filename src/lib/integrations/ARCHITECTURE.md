...
## First proposed live connector (after this UI pass — not built now)

SFTP / file-drop (not email, not AMS APIs):

1. Agency or carrier drops statement files via SFTP
2. Normalize statement lines (commission-feed contract)
3. Hand off to **existing** Reconciliation staging
4. Existing matching engine → review → receipt confirmation

Do not build a second reconciliation system. Email statement ingestion stays V2.
