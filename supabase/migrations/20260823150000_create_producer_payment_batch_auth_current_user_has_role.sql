-- Align create_producer_payment_batch_with_recoveries Owner/Admin authorization
-- with confirm_producer_paid_outside_alza_flow: current_user_has_role (users.role OR user_roles).
-- Behavior otherwise unchanged: draft batch, transactions stay ready, paid_date NULL,
-- recoveries still applied once at create. Do not mark paid.
...
BEGIN
  ...
  -- Lock + plan OPEN next_payout recoveries oldest-first (exclude direct_payment).
  -- NULL settlement_method is treated as next_payout (legacy / safe default).
  FOR v_rec IN
    SELECT
      r.id,
...
COMMENT ON FUNCTION public.create_producer_payment_batch_with_recoveries(text, uuid[], text) IS
  'Atomically creates a producer payment batch, applies OPEN next_payout recoveries oldest-first (excludes direct_payment), and links transactions. Owner/Admin via current_user_has_role (users.role or user_roles). Never marks paid.';
