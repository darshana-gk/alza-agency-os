-- Additive owner profile fields for public self-serve signup.
-- users.job_title is nullable so existing and invited users remain valid.
-- agency_profile.phone already exists and stays nullable.
-- Replaces self_serve_create_agency_owner with two optional trailing arguments.
-- Staging only. Does not rename agency architecture or alter customer operational tables.

ALTER TABLE public.users
  ADD COLUMN IF NOT EXISTS job_title text;

COMMENT ON COLUMN public.users.job_title IS
  'Owner/user job title or designation. Required for new self-serve signups; nullable for existing and invited users.';

DROP FUNCTION IF EXISTS public.self_serve_create_agency_owner(uuid, text, text, text, text, text, text, text);
...
BEGIN
  ...
  v_job_title := NULLIF(btrim(COALESCE(p_job_title, '')), '');
  v_phone := NULLIF(btrim(COALESCE(p_work_phone, '')), '');
...
REVOKE ALL ON FUNCTION public.self_serve_create_agency_owner(uuid, text, text, text, text, text, text, text, text, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.self_serve_create_agency_owner(uuid, text, text, text, text, text, text, text, text, text) FROM anon;
REVOKE ALL ON FUNCTION public.self_serve_create_agency_owner(uuid, text, text, text, text, text, text, text, text, text) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.self_serve_create_agency_owner(uuid, text, text, text, text, text, text, text, text, text) TO service_role;

COMMENT ON FUNCTION public.self_serve_create_agency_owner(uuid, text, text, text, text, text, text, text, text, text) IS
  'Self-serve: create one agency + Owner for a new Auth user. service_role only. Optional incomplete billing intent. Optional job title and work phone.';
