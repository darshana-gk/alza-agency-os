-- Exclude direct_payment recoveries from automatic producer payout application.
-- next_payout (and NULL/legacy) recoveries continue to apply oldest-first.
-- Signature unchanged: create_producer_payment_batch_with_recoveries(text, uuid[], text)
...
COMMENT ON FUNCTION public.create_producer_payment_batch_with_recoveries(text, uuid[], text) IS
  'Atomically creates a producer payment batch, applies OPEN next_payout recoveries oldest-first (excludes direct_payment), and links transactions. Owner/Admin only.';
