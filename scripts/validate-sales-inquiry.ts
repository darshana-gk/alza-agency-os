...
{
  const ok = validateSalesInquiryInput(valid)
  assert(ok.ok === true, 'valid submission accepted')
  assert(ok.ok === true && ok.value.jobTitle === 'Operations Manager', 'job title stored on valid payload')
  assert(ok.ok === true && ok.value.message === 'Need a walkthrough', 'message stored on valid payload')
  assert(ok.ok === true && ok.value.agencyName === 'Example Agency', 'company stored as agencyName internally')
  ...
  const missingEmail = validateSalesInquiryInput({ ...valid, workEmail: '' })
  assert(missingEmail.ok === false && 'code' in missingEmail && missingEmail.code === 'email_required', 'required email')
  const missingCompany = validateSalesInquiryInput({ ...valid, agencyName: '' })
  assert(missingCompany.ok === false && 'code' in missingCompany && missingCompany.code === 'agency_required', 'required company')
  const missingTitle = validateSalesInquiryInput({ ...valid, jobTitle: '' })
  assert(missingTitle.ok === false && 'code' in missingTitle && missingTitle.code === 'job_title_required', 'required job title')
  const missingBand = validateSalesInquiryInput({ ...valid, userBand: '' })
  assert(missingBand.ok === false && 'code' in missingBand && missingBand.code === 'user_band_required', 'required user band')
  const missingMessage = validateSalesInquiryInput({ ...valid, message: '' })
  assert(missingMessage.ok === false && 'code' in missingMessage && missingMessage.code === 'message_required', 'required message')
  const whitespaceMessage = validateSalesInquiryInput({ ...valid, message: '   \n\t  ' })
  assert(
    whitespaceMessage.ok === false && 'code' in whitespaceMessage && whitespaceMessage.code === 'message_required',
    'whitespace-only message rejected',
  )
  const shortMessage = validateSalesInquiryInput({ ...valid, message: 'too short' })
  assert(
    shortMessage.ok === false && 'code' in shortMessage && shortMessage.code === 'message_too_short',
    'message shorter than minimum rejected',
  )
  assert(SALES_INQUIRY_LIMITS.messageMin === 10, 'message minimum is 10')
  ...
  const longTitle = validateSalesInquiryInput({ ...valid, jobTitle: 'x'.repeat(SALES_INQUIRY_LIMITS.jobTitle + 1) })
  assert(longTitle.ok === false, 'oversized job title rejected')
...
console.log('D. Route, form, storage, anti-spam architecture')
{
  ...
  assert(contact.includes('Company Name'), 'company name public label')
  assert(!contact.includes('Agency Name'), 'agency name public label removed')
  assert(contact.includes('Job Title / Designation'), 'job title field')
  assert(contact.includes('e.g. Agency Owner, Operations Manager'), 'job title free-text placeholder')
  assert(!contact.includes('<select') || contact.indexOf('Job Title / Designation') < contact.indexOf('Number of Users'), 'job title before user band')
  assert(contact.includes('Number of Users'), 'users field')
  assert(contact.includes('Message / What can we help with?'), 'message field')
  assert(contact.includes('htmlFor="message" required'), 'message marked required')
  assert(!contact.includes('Message / What can we help with?</') && !/label="Message[^"]*" htmlFor="message">/.test(contact), 'message no longer optional')
  ...
  assert(fn.includes('New ALZA Flow Inquiry'), 'ALZA notification subject')
  assert(fn.includes('Job Title / Designation:'), 'notification includes job title')
  assert(fn.includes('Company: ${agencyName}'), 'notification includes company')
  assert(fn.includes('Message: ${message}'), 'notification includes message')
  assert(!fn.includes('Agency Name:'), 'notification no longer labels Agency Name')
  assert(fn.includes('job_title: jobTitle'), 'persists job_title')
  assert(fn.includes('job_title_required'), 'server requires job title')
  assert(fn.includes('message_required'), 'server requires message')
  assert(fn.includes('message_too_short'), 'server rejects short message')
  assert(fn.includes('source_invalid'), 'server rejects invalid source')
  assert(fn.includes('messageMin: 10'), 'server message minimum is 10')
  ...
  const jobTitleMigration = read('supabase/migrations/20260917180000_sales_inquiries_job_title.sql')
  assert(jobTitleMigration.includes('ADD COLUMN IF NOT EXISTS job_title'), 'additive job_title migration')
  assert(!jobTitleMigration.includes('DROP COLUMN'), 'job title migration is not destructive')
  assert(!jobTitleMigration.includes('RENAME COLUMN agency_name'), 'agency_name column kept')
