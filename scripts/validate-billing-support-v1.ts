...
console.log('G2. Legacy-active + V2 catalog visibility (no second checkout)')
{
  ...
  assert(page.includes('Proceed to Secure Checkout'), 'purchasable CTA Proceed to Secure Checkout')
  ...
  assert(page.includes("disabled={busy || processing || planLocked || product === 'alza_flow_pay'}"), 'Flow Pay frequency toggle disabled')
  ...
  assert(page.includes('Proceed to Secure Checkout'), 'purchase CTA in quote panel')
